namespace Notepads.Extensions {
  ...
  public static class StringExtensions {
    ...
    public static int IndexOfWholeWord(this string target, string value,
                                       int startIndex,
                                       StringComparison comparison) {
      int pos = startIndex;

      while (pos < target.Length &&
             (pos = target.IndexOf(value, pos, comparison)) != -1) {
    ...
    public static int LastIndexOfWholeWord(this string target, string value,
                                           int startIndex,
                                           StringComparison comparison) {
      int pos = startIndex;

      while (pos >= 0 &&
             (pos = target.LastIndexOf(value, pos, comparison)) != -1) {
        bool startBoundary = true;
        if (pos > 0)
          startBoundary = !char.IsLetterOrDigit(target[pos - 1]);
    ...
    public static Uri ToAppxUri(this string path) {
      string prefix =
          $"ms-appx://{(path.StartsWith('/') ? string.Empty : " / ")}";
