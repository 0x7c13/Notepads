namespace Notepads.Controls.TextEditor {
  ...
  public partial class TextEditorCore {
    private readonly Dictionary<int, TextBlock>_renderedLineNumberBlocks =
        new Dictionary<int, TextBlock>();

    private void OnContentScrollViewerSizeChanged(object sender,
                                                  SizeChangedEventArgs e) {
      RenderLineNumbers();
    }

    private void OnLineNumberGridSizeChanged(object sender,
                                             SizeChangedEventArgs e) {
    ...
    private void ResetLineNumberCanvasClipping() {
      RectangleGeometry rectangle = new RectangleGeometry{
          Rect = new Rect(
              0, Padding.Top, _lineNumberGrid.ActualWidth,
              Math.Clamp(_lineNumberGrid.ActualHeight - (Padding.Top * 2), .0f,
    ...
    private void RenderLineNumbers() {
      ...
      var startRange = Document.GetRangeFromPoint(
          new Point(_contentScrollViewer.HorizontalOffset,
                    _contentScrollViewer.VerticalOffset),
          PointOptions.ClientCoordinates);

      var endRange = Document.GetRangeFromPoint(
          new Point(_contentScrollViewer.HorizontalOffset +
                        _contentScrollViewer.ViewportWidth,
                    _contentScrollViewer.VerticalOffset +
                        _contentScrollViewer.ViewportHeight),
          PointOptions.ClientCoordinates);

      var lines = _content.Split(RichEditBoxDefaultLineEnding);

      Dictionary<int, Rect>lineRects =
          CalculateLineRects(lines, startRange, endRange);

      RenderLineNumbersInternal(lineRects);

      _lineNumberGrid.Width =
          FontUtility
              .GetTextSize(FontFamily, FontSize, (lines.Length).ToString())
              .Width +
          Padding.Right;
    }

    private Dictionary<int, Rect>CalculateLineRects(string[] lines,
      ...
      foreach(var line in lines) {
        if (offset >= startRange.StartPosition &&
            offset <= endRange.EndPosition) {
    ...
    private void RenderLineNumbersInternal(Dictionary<int, Rect>lineRects) {
      ...
      foreach(var lineRect in lineRects) {
        var margin =
            new Thickness(Padding.Left, lineRect.Value.Top + Padding.Top + 2,
                          Padding.Right, 0);
        ...
        if (_renderedLineNumberBlocks.ContainsKey(lineRect.Key)) {
          ...
          _renderedLineNumberBlocks[lineRect.Key].Visibility =
              Visibility.Visible;
        } else {
          var lineNumberBlock = new TextBlock(){
              ...
              Foreground = new SolidColorBrush(
                  (Color)
                      Application.Current.Resources["SystemBaseMediumColor"])};
      ...
      foreach(var numberBlock in _renderedLineNumberBlocks.Where(
          x => !lineRects.ContainsKey(x.Key))) {
