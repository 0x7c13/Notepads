  ...
  public static class ThemeSettingsService {
    ...
    public static bool UseWindowsTheme {
      ...
      set {
        if (value != _useWindowsTheme) {
          ...
          ApplicationSettingsStore.Write(SettingsKey.UseWindowsThemeBool,
                                         _useWindowsTheme);
        }
      }
    }
    ...
    public static bool UseWindowsAccentColor {
      ...
      set {
        ...
        ApplicationSettingsStore.Write(SettingsKey.UseWindowsAccentColorBool,
                                       _useWindowsAccentColor);
      }
    }
    ...
    public static double AppBackgroundPanelTintOpacity {
      ...
      set {
        ...
        ApplicationSettingsStore.Write(
            SettingsKey.AppBackgroundTintOpacityDouble, value);
      }
    }
    ...
    public static Color AppAccentColor {
      ...
      set {
        ...
        ApplicationSettingsStore.Write(SettingsKey.AppAccentColorHexStr,
                                       value.ToHex());
      }
    }
    ...
    public static Color CustomAccentColor {
      ...
      set {
        ...
        ApplicationSettingsStore.Write(SettingsKey.CustomAccentColorHexStr,
                                       value.ToHex());
      }
    }
    ...
    private static void InitializeAppAccentColor() {
      if (ApplicationSettingsStore.Read(SettingsKey.UseWindowsAccentColorBool)
              is bool useWindowsAccentColor) {
      ...
      _appAccentColor = UISettings.GetColorValue(
          Windows.UI.ViewManagement.UIColorType.Accent);

      if (!UseWindowsAccentColor) {
        if (ApplicationSettingsStore.Read(SettingsKey.AppAccentColorHexStr)
                is string accentColorHexStr) {
    ...
    private static void InitializeCustomAccentColor() {
      if (ApplicationSettingsStore.Read(SettingsKey.CustomAccentColorHexStr)
              is string customAccentColorHexStr) {
    ...
    private static void UiSettings_ColorValuesChanged(UISettings sender,
                                                      object args) {
      if (UseWindowsAccentColor) {
    ...
    private static void InitializeAppBackgroundPanelTintOpacity() {
      if (ApplicationSettingsStore.Read(
              SettingsKey.AppBackgroundTintOpacityDouble)
              is double tintOpacity) {
    ...
    private static void InitializeThemeMode() {
      if (ApplicationSettingsStore.Read(SettingsKey.UseWindowsThemeBool)
              is bool useWindowsTheme) {
      ...
      if (!UseWindowsTheme) {
        if (ApplicationSettingsStore.Read(SettingsKey.RequestedThemeStr)
                is string themeModeStr) {
          if (Enum.TryParse(typeof (ElementTheme), themeModeStr,
                            out var theme)) {
    ...
    public static void SetTheme(ElementTheme theme) {
      if (ThemeMode != theme) {
        ThemeMode = theme;
        ApplicationSettingsStore.Write(SettingsKey.RequestedThemeStr,
                                       ThemeMode.ToString());
        OnThemeChanged?.Invoke(null, theme);
      }
    }

    public static void SetRequestedTheme(Panel backgroundPanel,
      ...
      ((SolidColorBrush) Application.Current
           .Resources["SystemControlPageBackgroundMediumAltMediumBrush"])
          .Color =
          ThemeMode == ElementTheme.Dark ? Color.FromArgb(153, 0, 0, 0)
    ...
    private static Brush GetAppBackgroundBrush(ElementTheme theme) {
      ...
      var baseColor =
          theme == ElementTheme.Light ? lightModeBaseColor : darkModeBaseColor;
      ...
      if (AppBackgroundPanelTintOpacity > 0.99f ||
        ...
        if (_currentAppBackgroundBrush is HostBackdropAcrylicBrush
                hostBackdropAcrylicBrush) {
          hostBackdropAcrylicBrush.LuminosityColor = baseColor;
          hostBackdropAcrylicBrush.TintOpacity =
              (float) AppBackgroundPanelTintOpacity;
          return _currentAppBackgroundBrush;
        }
        return _currentAppBackgroundBrush =
                   BrushUtility
                       .GetHostBackdropAcrylicBrush(
    ...
    public static void
    ApplyThemeForTitleBarButtons(ApplicationViewTitleBar titleBar,
                                 ElementTheme theme) {
    ...
    private static void UpdateSystemAccentColorAndBrushes(Color color) {
      ...
      // Took from:
      // https://stackoverflow.com/questions/31831917/change-accent-color-in-windows-10-uwp/31844773
      var brushes =
          new string[]{"SystemControlBackgroundAccentBrush",
      ...
      foreach(var brush in brushes) {
        try {
          ((SolidColorBrush) Application.Current.Resources[brush]).Color =
              color;
        } catch (Exception ex) {
          LoggingService.LogError(
              $"[{nameof(ThemeSettingsService)}] Failed to apply color change for Brush: [{brush}]: {ex.Message}");
        }
      }

      try {
        // Overwrite MenuFlyoutSubItemRevealBackgroundSubMenuOpened resource
        // color
        ((RevealBackgroundBrush) Application.Current
             .Resources["SystemControlHighlightAccent3RevealBackgroundBrush"])
            .Color = color;
      } catch (Exception) {
        // ignore
      }
